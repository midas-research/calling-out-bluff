...
#   #convert this list to a string. row 6 gives score and 2 gives content
#   #prtint(z)
#   #y = z.split(', ', 1)
#   #print(y)
#     scorelist.append(score)
#     essaylist.append(content)
...
test_set = pd.read_csv('/home/mehar/github/ease/aes_data/essay1/fold_0/test.txt', sep='\t')
x = test_set.to_numpy()
tester = x.tolist()
...
for i in range(0, len(tester)):
    print("index", i)
    tokens = tester[i]
    content_test = tokens[2].strip()
    score_test = float(tokens[6])

  #z = tester[i]
  #y = z[0].split(', ', 1)
    test_scorelist.append(score_test)
    test_essaylist.append(content_test)
count = 0
...
final = clf.predict(Y)   
## INVERSE_SCALING
finals = np.rint(np.clip(final,a_min=0,a_max=1)*12)

## QWK Score
quadratic_weighted_kappa(test_scorelist,finals) ## test_scorelist   babble_score    gpt_score
