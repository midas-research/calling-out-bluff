...
def read_dataset_into_tfrecord(dataset_path, bw):
    dataset_positive_path = os.path.join(datasset_path, "prompt7.csv")
    print("DSS", dataset_positive_path)
    tf_record_path = os.path.join(dataset_path, "asap_dataset_prompt7.tfrecord")
...
def read_ielts_into_tfrecord(dataset_path, bw):
...
def input_fn_from_tfrecord(tfrecord_path, batch_size, is_training, element_ids):
    ...
    prompts_embedding_path = os.path.join(sys_conf["data_dir"], "prompt7.npz")
...
def read_asap_dataset():
    ...
    asap_csv_file_path = "training_set_rel3.csv"
    ...
    asap_dataset = pd.read_csv("training_set_rel3.csv", encoding='ISO-8859-1')
...
#             print("ID", id)
            score = line_split[6].strip()
            print("score", score)
#             print(line_split[0], line_split[2])
#             id = int(line_split[0].strip())
            gec_output = line_split[2].strip()
#             print(gec_output)
            
            train = EssaySet()
            train.add_essay(gec_output, score)
            feats = FeatureExtractor()
            feats.initialize_dictionaries(train)
            features[id] = feats.gen_feats(train) 
...
#     read_dataset_into_tfrecord("dataset/", bw)
    ...
    generate_xgboost_train_set(articles_id, articles_set, domain1_score, "training_set_rel3.tsv", "asap_xgboost_promptNEW.npz")
    print("Done")
