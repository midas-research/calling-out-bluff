...
if __name__ == "__main__":
	...
	# 			bkp=dev_kappa_result
	# 			# model.save_weights(f"../checkpoints/save_one_{args.set_id}.ckpt")
	# 			torch.save(model.state_dict(), f"../checkpoints/save_one_{args.set_id}.ckpt")
	# # torch.cuda.empty_cache()

	# model.load_state_dict(torch.load(f"../checkpoints/save_one_{args.set_id}.ckpt"))
	# ig = IntegratedGradients(model.forward_ig)

	# mid1 = round(n_test/3)
	# mid2 = round(n_test/3)*2
	# test_batches = [(0, mid1), (mid1, mid2), (mid2, n_test)]
	# all_pred_scores = []
	# for start, end in test_batches:
	# 	word_wise_attributions = get_attributes(dev_contents_idx[start:end], [memory_contents]*test_contents.shape[0], ig, model)
	...
	# # contents_pt= (torch.from_numpy(contents).to(device).requires_grad_(False))
	# # contents_pt= (torch.from_numpy(contents).to(device).detach())
	# contents_pt = Variable(torch.from_numpy(contents).to(device))
	# # batched_memory_contents_pt = (torch.from_numpy(batched_memory_contents).to(device).requires_grad_(False))
	# # batched_memory_contents_pt = (torch.from_numpy(batched_memory_contents).to(device).detach())
	# batched_memory_contents_pt = Variable(torch.from_numpy(batched_memory_contents).to(device))
	# pred_score = model.test(contents, batched_memory_contents) 
	# embeddings = model.word_to_vec(contents_pt)
	# memories = model.word_to_vec(batched_memory_contents_pt)
	...
	# attributions, approximation_error = ig.attribute((Variable(embeddings), Variable(memories)), target=pred_score)
	# word_wise_attributions = attributions.sum(dim=2)
	# print(word_wise_attributions.shape)
